  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "login") {
        ...
        // En un escenario real, aquí guardaríamos el 'role' en Firestore después de crear el usuario
        await createUserWithEmailAndPassword(auth, email, password)
        toast({
          title: "Cuenta creada",
          description: `Usuario registrado como ${role === 'admin' ? 'Administrador' : 'Usuario'}.`,
        })
      }
      router.push("/")
    } catch (error: any) {
      ...
      if (error.code === 'auth/wrong-password' || error.code === 'auth/user-not-found') {
        ...
        message = "Error de configuración de Firebase. Verifique la API Key."
      }
              ...
              <CardContent className="space-y-6">
                ...
                {mode === "register" && (
                  <div className="space-y-3 pt-2">
                    <Label className="text-sm font-bold">Tipo de Rol</Label>
                    <RadioGroup 
                      value={role} 
